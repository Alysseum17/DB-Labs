...
```sql
SELECT q.quiz_id, q.title, COUNT(qa.attempt_id) AS attempt_count
FROM quizes q
...
```sql
SELECT q.quiz_id, q.title, AVG(r.rating) AS average_rating
FROM quizes q
...
```sql
SELECT u.user_id, u.username, COUNT(q.quiz_id) AS quiz_count
FROM users u
...
```sql
SELECT q.quiz_id, q.title, qs.question_type, COUNT(qs.question_id) AS question_count
FROM quizes q
...
```sql
SELECT u.user_id, u.username AS author_name, AVG(r.rating) AS average_rating_of_quizes
FROM users u
INNER JOIN quizes q ON u.user_id = q.author_id
...
```sql
SELECT u.user_id, u.username, AVG(qa.score) AS average_score_on_easy
FROM users u
...
```sql
SELECT u.user_id, u.username AS author_name, AVG(uqs.best_score) AS average_best_score
FROM users u
INNER JOIN quizes q ON u.user_id = q.author_id
...
### 1.8 Quizzes with ≥ 5 Attempts
```sql
SELECT q.quiz_id, q.title, COUNT(qa.attempt_id) AS attempts_count
FROM quizes q
...
### 1.9 Authors with ≥ 3 Quizzes
```sql
SELECT u.user_id, u.username, COUNT(q.quiz_id) AS quizes_count
FROM users u
INNER JOIN quizes q ON u.user_id = q.author_id
GROUP BY u.user_id, u.username
...
```sql
SELECT q.quiz_id, q.title, SUM(qs.points) AS total_possible_score
FROM quizes q
...
### 2.3 Users Who Took > 2 Quizzes
```sql
SELECT u.user_id, u.username, COUNT(uqs.quiz_id) AS quizes_taken_count
FROM users u
...
### 2.4 Score Statistics for Quiz #2
```sql
...
### 2.6 Attempts and Scores for Quiz #1
```sql
...
### 2.9 Average Score per User (Subquery in SELECT)
```sql
SELECT u.username,
       (SELECT AVG(qa.score)
        FROM quiz_attempts qa
...
## Summary
This revised version now:
- **Includes primary keys** (`quiz_id`, `user_id`) for unambiguous grouping and joins.  
- Demonstrates all required **OLAP operations**: aggregation, subqueries, and multi-table joins.  
- Preserves full analytical coverage of user, quiz, and scoring data with consistent naming and sorting logic.
