 ...
 **Вимога:** Атомарність атрибутів, відсутність повторюваних груп 
  ...
  * **Перевірка:** У початковій схемі відсутні списки значень (наприклад, варіанти відповідей не записані в одну клітинку через кому, як "A, B, C"). Вони винесені в окрему таблицю `AnswerOption`.
 ...
 **Вимога:** Відсутність часткових залежностей (неключові атрибути залежать від усього складеного ключа) 
  ...
  * **Перевірка:**
      ...
      * Таблиця `SelectedAnswer` має складений ключ (`question_response_id`, `answer_option_id`), але вона слугує лише для зв'язку (link table) і не має додаткових атрибутів, що залежали б від частини ключа.
 ...
 **Вимога:** Відсутність транзитивних залежностей (неключовий атрибут залежить від іншого неключового атрибута) 
  ...
  * **Перевірка:** У таблиці `Review` є `user_id` та `quiz_id`. Текст відгуку залежить від ID відгуку (`review_id`), а не від користувача чи вікторини окремо. Ми не зберігаємо `author_name` у таблиці `Quize`, що усуває транзитивну залежність (ID $\rightarrow$ AuthorID $\rightarrow$ AuthorName).
...
**Проблемна таблиця:** `UserQuizStat`
...
1.  **Аномалія оновлення:** Якщо адміністратор видалить запис про спробу проходження тесту з таблиці `QuizAttempt`, значення `Attempt` та `best_score` у таблиці `UserQuizStat` стануть некоректними. Це вимагає складних тригерів для синхронізації.
2.  **Надлишковість даних:** Ми витрачаємо пам'ять на зберігання даних, які вже існують у системі, просто в іншому вигляді (у таблиці `QuizAttempt`).
...
```sql
DROP TABLE IF EXISTS UserQuizStat;
```
...
1.  **Усунено надлишковість:** Дані про бали зберігаються лише в `QuizAttempt`.
...
* **Потенційна помилка:** Збереження варіантів відповідей для питань типу `multiple_choice` у вигляді списку в одному стовпці (наприклад, стовпець `options` зі значенням "A, B, C") або створення стовпців `option_1`, `option_2`, `option_3` у таблиці `Question`. Це порушує вимогу атомарності атрибутів та заборону на повторювані групи.
* **Рішення в схемі:** Створено окрему таблицю `AnswerOption`, де кожен варіант відповіді зберігається в окремому рядку та посилається на `question_id`.
...
* **Потенційна помилка:** У таблиці `SelectedAnswer` використовується складений первинний ключ `(question_response_id, answer_option_id)`. [cite_start]Якби ми додали в цю таблицю стовпець `is_correct` (чи є відповідь правильною), це створило б порушення 2NF[cite: 18]. Значення `is_correct` залежить лише від частини ключа — `answer_option_id` (самого варіанту), і ніяк не залежить від конкретної спроби відповіді (`question_response_id`).
* **Рішення в схемі:** Атрибут `is_correct` залишено виключно в батьківській таблиці `AnswerOption`, де він залежить від повного ключа. Таблиця `SelectedAnswer` містить лише зовнішні ключі, необхідні для зв'язку.
...
* **Потенційна помилка:** Розміщення інформації про автора (наприклад, `author_email` або `author_username`) безпосередньо в таблиці `Quize`. У такому випадку виникла б транзитивна залежність: `quiz_id` $\rightarrow$ `author_id` $\rightarrow$ `author_email`. Неключовий атрибут `author_email` залежав би від іншого неключового атрибута `author_id`. Це призвело б до аномалій оновлення: при зміні пошти користувача довелося б оновлювати всі його вікторини.
* **Рішення в схемі:** Інформація про автора зберігається виключно в таблиці `User`. Таблиця `Quize` містить лише зовнішній ключ `author_id`.
